...
class R2U_Net(nn.Module):
    ...
    def __init__(self, img_ch=3, output_ch=1, t=2):
        ...
        self.layer5 = ConvDoubleBlock(filters[3],filters[3])
        self.layer6 = ConvDoubleBlock(filters[4],filters[3])
...
class ConvDoubleBlock(nn.Module):
    ...
    def __init__(self, in_num_ch, out_num_ch, filter_size=3):
        ...
        self.conv = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Conv2d(out_num_ch, out_num_ch, filter_size, padding=1,stride = 1),
            nn.BatchNorm2d(out_num_ch),
            nn.ReLU(inplace=True)
            )
...
if __name__ == "__main__":
    ...
    net = (1,1).cpu()#R2U_Net
    #net.load_state_dict(torch.load('model_path'+'best_model_{}_final.path'.format(net.__class__.__name__),map_location = 'cpu'))
    result = net(X)